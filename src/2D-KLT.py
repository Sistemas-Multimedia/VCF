...
parser.parser_encode.add_argument("-B", "--block_size", type=parser.int_or_str, help=f"Tamaño de bloque (por defecto: {default_block_size})", default=default_block_size)
...
parser.parser_decode.add_argument("-B", "--block_size", type=parser.int_or_str, help=f"Tamaño de bloque (por defecto: {default_block_size})", default=default_block_size)
...
class CoDec(CT.CoDec):
    ...
    def __init__(self, args):
        ...
        try:
            ...
            if not hasattr(args, 'block_size'):
                raise AttributeError("args debe tener atributo 'block_size'")
            ...
            if isinstance(block_size, str):
                try:
                    ...
                    raise ValueError(f"block_size no puede convertirse a entero: {block_size}")

            if not isinstance(block_size, (int, np.integer)) or block_size <= 0:
                raise ValueError(f"block_size debe ser entero positivo, recibido: {block_size}")
    ...
    def encode_fn(self, in_fn, out_fn):
        ...
        try:
            ...
            np.savez_compressed(f"{out_fn}_weights.npz", weights=lbt_weights)
    ...
    def decode_fn(self, in_fn, out_fn):
        ...
        try:
            ...
            files_to_check = [in_fn + ".tif", f"{in_fn}_shape.bin", f"{in_fn}_weights.npz"]
            ...
            try:
                ...
                with np.load(f"{in_fn}_weights.npz") as data:
                ...
                raise IOError(f"Error al leer archivo de pesos: {in_fn}_weights.npz: {e}")
