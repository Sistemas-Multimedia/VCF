'''Entropy Encoding of images non-adaptive Arithmetic Coding'''
...
import os
import math
from arithmetic_coding import arithmetic_coding # pip install --ignore-installed "arithmetic_coding @ git+https://github.com/vicente-gonzalez-ruiz/arithmetic_coding"
...
class CoDec(EIC.CoDec):
    ...
    def compress_fn(self, img, fn):
        ...
        stats_fn = f"{fn}_arith_CMD.pkl.gz"
        compressed_img = io.BytesIO()

        # Find unnormalized CMD (Cummulative Mass Distribution)
        flat_img = img.flatten().astype(np.int32)
        # Returns in symbols the sorted list of found symbols and in unnormalized PMF (Probability Mass Function) of the input file
        symbols, PMF = np.unique(flat_img, return_counts=True)
        CMD = np.cumsum(PMD)
